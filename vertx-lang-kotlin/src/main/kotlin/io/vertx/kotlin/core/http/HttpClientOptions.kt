 ...
 * @param http2UpgradeMaxContentLength  Set the HTTP/2 upgrade maximum length of the aggregated content in bytes. This is only taken into account when [io.vertx.core.http.HttpClientOptions] is set to <code>false</code> (which is the default). When [io.vertx.core.http.HttpClientOptions] is <code>true</code>, then the client makes a preflight OPTIONS request and the upgrade will not send a body, voiding the requirements.
