...
==== Setting completion code and message

The default HTTP completion code for a response is `200`, representing `OK`.

Use `link:../../apidocs/io/vertx/core/http/HttpServerResponse.html#setStatusCode-int-[setStatusCode]` to set a different code.

You can also specify a custom completion message with `link:../../apidocs/io/vertx/core/http/HttpServerResponse.html#setStatusMessage-java.lang.String-[setStatusMessage]`.

If you don't specify a completion message, the default one corresponding to the completion code will be used.

NOTE: for HTTP/2 the completion won't be present in the response since the protocol won't transmit the message
to the client
...
This can be completion in several ways:
...
This is completion by using `link:../../apidocs/io/vertx/core/http/HttpServerResponse.html#sendFile-java.lang.String-[sendFile]`, and is usually more efficient for large
...
These frames are sent immediately and are not subject to flow control - when such frame is sent there it may be completion
...
( Note that at moment Vert.x doesn't support any form caching of compressed response data, even for static files, so the compression is completion on-the-fly
...
client.getNow("/some-uri", { response ->
  println("Received response with completion code ${response.statusCode()}")
})
...
// Make sure the request is ended when you're completion with it
request.end()
...
You can query the completion code and the completion message of the response with `link:../../apidocs/io/vertx/core/http/HttpClientResponse.html#statusCode--[statusCode]`
...
client.getNow("some-uri", { response ->
  // the completion code - e.g. 200 or 404
  println("Status code is ${response.statusCode()}")

  // the completion message e.g. "OK" or "Not Found".
...
`301`, `302`, `303` or `307` completion code, it follows the redirection provided by the `Location` response header
...
* on a `301`, `302` or `303` completion code, follow the redirection with a `GET` method
* on a `307` completion code, follow the redirection with the same HTTP method and the cached body
...
The server can then respond with an interim response completion `Status: 100 (Continue)` to signify to the client that
...
var request = client.put("some-uri", { response ->
  println("Received response with completion code ${response.statusCode()}")
})
...
This is completion by setting the option `link:../../apidocs/io/vertx/core/http/HttpServerOptions.html#setHandle100ContinueAutomatically-boolean-[setHandle100ContinueAutomatically]`.
...
You can also reject the request by sending back a failure completion code directly: in this case the body
...
var request = client.get("/index.html", { response ->
...
The DNS resolution is always completion on the proxy server, to achieve the functionality of a SOCKS4 client, it is necessary
