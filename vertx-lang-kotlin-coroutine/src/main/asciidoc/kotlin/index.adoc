...
This is done by using the `awaitResult` method which returns the value or throws an exception.
...
Processing a one-shot event (and not the next occurrences, if any) is done using the `awaitEvent` function:
