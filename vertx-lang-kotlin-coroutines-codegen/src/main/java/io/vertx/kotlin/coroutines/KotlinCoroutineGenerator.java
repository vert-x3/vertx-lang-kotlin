...
import io.vertx.kotlin.coroutines.util.Writer;
...
public class KotlinCoroutineGenerator extends Generator<ClassModel> {
  ...
  private void generateMethod(ClassTypeInfo type, MethodInfo method, Writer writer, boolean hasStatic){
    ...
    if (!method.isStaticMethod()) {
      ...
      if (!type.getParams().isEmpty()) {
        writer.print(type.getParams().stream().map(TypeParamInfo::getName).collect(Collectors.joining(",", "<", ">")));
      }
      writer.print(".");
    }
    ...
    writer.print(params.stream().limit(params.size() - 1).map(p -> p.getName() + " : " + kotlinType(p.getType())).collect(Collectors.joining(", ")));
    writer.print(")");
    ParameterizedTypeInfo lastParamType = (ParameterizedTypeInfo) params.get(params.size() - 1).getType();
    ...
    writer.print("(");
    writer.print(params.stream().limit(params.size() - 1).map(p -> keyWordConverter(p.getName())).collect(Collectors.joining(", ")));
  ...
  private boolean generateFilter(MethodInfo it) {
    ...
    return (it.isFluent() || "void".equals(it.getReturnType().getName())) && (methodKind == MethodKind.HANDLER || methodKind == MethodKind.FUTURE);
  }
